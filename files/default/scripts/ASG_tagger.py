...
'''
Keys should be:
- SSR_managed = True/False - can be forced to False in attrs. Checked every time.
        ...
        try:
            ...
            for LC in spot_LCs:
                ...
                for as_group in spot_LC_groups:
                    ...
                    if not [ t for t in as_group.tags if t.key == 'SSR_managed' ]:
                        # this group does not have the SSR_managed tag indicator
                        ...
                        pass
                    elif [ t for t in as_group.tags if t.key == 'SSR_managed' and t.value == 'False' ]:
                        # a group that we shouldn't manage
                        print_verbose('Not managing group as SSR_managed set to false.' % as_group.name, verbose)
                        pass
                    elif [ t for t in as_group.tags if t.key == 'SSR_managed' and t.value == 'True' ]:
                        ...
                        raise Exception("SSR_managed tag found for %s but isn't a valid value." % as_group.name)
